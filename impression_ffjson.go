...
handle_Instl:
	...
	/* handler: uj.Instl type=openrtb.NumberOrBool kind=int quoted=false*/

	{
		...
		err = uj.Instl.UnmarshalJSON(tbuf)
		if err != nil {
			return fs.WrapErr(err)
		}
		state = fflib.FFParse_after_value
...
handle_Secure:
	...
	/* handler: uj.Secure type=openrtb.NumberOrBool kind=int quoted=false*/
