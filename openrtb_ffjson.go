...
handle_UTCOffset:
	...
	/* handler: uj.UTCOffset type=openrtb.NumberOrString kind=int quoted=false*/

	{
		...
		err = uj.UTCOffset.UnmarshalJSON(tbuf)
		if err != nil {
			return fs.WrapErr(err)
		}
		state = fflib.FFParse_after_value
...
handle_Coppa:
	...
	/* handler: uj.Coppa type=openrtb.NumberOrBool kind=int quoted=false*/

	{
		...
		err = uj.Coppa.UnmarshalJSON(tbuf)
		if err != nil {
			return fs.WrapErr(err)
		}
		state = fflib.FFParse_after_value
